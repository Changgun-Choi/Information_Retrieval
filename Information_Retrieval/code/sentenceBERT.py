...
train_pos = pd.read_csv(path + "triples.train.small.tsv",
                        sep="\t", nrows=1000, header=None, skiprows=1, usecols=[0, 1])
train_neg = pd.read_csv(path + "triples.train.small.tsv",
                        sep="\t", nrows=1000, header=None, skiprows=1, usecols=[0, 2])
...
for i in range(0, 999):
    if save_array[i][2] > save_array[i + 1000][2]:
        # print("yes", save_array[i][2], save_array[i + 10000][2])
        count_right = count_right + 1
    else:
        print("no", save_array[i][2], save_array[i + 1000][2])
